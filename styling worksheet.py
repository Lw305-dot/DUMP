...
output_dir = Path("/workspaces/DUMP/Employee_Reports28")
...
ROW_RED_FILL = PatternFill(fill_type="solid", start_color="FFC7CE", end_color="FFC7CE")  # light red
...
def style_sheet(ws):
    ...
    if status_col:
        for r in range(3, ws.max_row + 1):
            ...
            if str(status_val).strip().upper() == "NOT VALID":
...
sheet_names = ["Cargo Trainings", "DFW Trainings", "AMH Trainings", "CBF Trainings", "SOPs","QNL Trainings","EXAMS"]
all_dfs = {}
...
for _, emp in employees.iterrows():
    ...
    if not exam_df.empty:
        ...
        try:
            avg = pd.to_numeric(exam_df["MARKS ATTAINED"], errors="coerce").mean()
