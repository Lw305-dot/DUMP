...
output_dir = Path("/workspaces/DUMP/Employee_Reports25")
trainingid_dir= Path("/workspaces/DUMP/Employee_Reports25/TrainingIDs")
qr_dir = Path("/workspaces/DUMP/Employee_Reports25/QR_Codes")
...
def shorten_name(emp_name, max_parts=3):
    """
    Keep only the first 'max_parts' words of the name.
    """
...
def create_id_card(emp_no, emp_name, qr_file_path, logo_path):
    ...
    total_text_block_height = title_height + spacing_between_lines + staff_height + spacing_between_lines + name_height
    ...
    # === Calculate Y alignment for centering ===
    # We want the whole logo + text block vertically centered together
    top_section_height = max(logo_img.height, total_text_block_height)
    top_y = MARGIN  # fixed top margin
    ...
    # Text block placement (aligned with logo center)
    text_x = logo_x + logo_img.width + 12
    text_y = top_y + (top_section_height - total_text_block_height) // 2

    # Draw texts vertically stacked
    draw.text((text_x, text_y), title_text, font=title_font, fill="black")
    draw.text((text_x, text_y + title_height + spacing_between_lines), staff_text, font=label_font, fill="black")
    draw.text((text_x, text_y + title_height + spacing_between_lines + staff_height + spacing_between_lines),
              name_text, font=label_font, fill="black")

    # --- Photo & QR side by side ---
    images_y = CARD_HEIGHT - IMAGE_SIZE - 30
    total_width = photo_img.width + qr_img.width + GAP
    start_x = (CARD_WIDTH - total_width) // 2

    photo_x = start_x
    qr_x = photo_x + photo_img.width + GAP
    ...
    footer_y = CARD_HEIGHT - MARGIN - 12
