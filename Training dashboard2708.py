...
output_dir = Path("/workspaces/DUMP/Employee_Reports22")
trainingid_dir= Path("/workspaces/DUMP/Employee_Reports22/TrainingIDs")
qr_dir = Path("/workspaces/DUMP/Employee_Reports22/QR_Codes")
...
def create_id_card(emp_no, emp_name, qr_file_path, logo_path):
    ...
    # Resize images
    photo_img = photo_img.resize((250, 250))
    qr_img = qr_img.resize((250, 250))
    logo_img.thumbnail((180, 80)) 

    # Create blank card (ID card size can be around CR80 standard ~ 1012x638 px)
    ...
    # Place logo at the top center
    card.paste(logo_img, (20, 20), logo_img)
    ...
    id_text = f"Staff ID: {emp_no}"
    name_text = f"Name: {emp_name}"

    bbox = draw.textbbox((0,0), id_text, font=label_font)
    id_w, id_h = bbox[2] - bbox[0], bbox[3] - bbox[1]
    id_x = (card_width - id_w) // 2
    id_y = title_y + title_h + 10   # 10px gap under title
    draw.text((id_x, id_y), id_text, font=label_font, fill="black")

    # Name
    bbox = draw.textbbox((0,0), name_text, font=label_font)
    name_w, name_h = bbox[2] - bbox[0], bbox[3] - bbox[1]
    name_x = (card_width - name_w) // 2
    name_y = id_y + id_h + 5        # 5px gap under Staff ID
    draw.text((name_x, name_y), name_text, font=label_font, fill="black")

    # Place photo (left) and QR (right)
    card.paste(photo_img, (60, 200))
    card.paste(qr_img, (380, 200))

    # Employee details under photo
    # draw.text((60, 410), f"Staff ID: {emp_no}", font=label_font, fill="black")
    # draw.text((60, 440), f"Name: {emp_name}", font=label_font, fill="black")

    # Footer
    footer_text = "@copyright Lodige Systems Middle East"
    bbox = draw.textbbox((0,0), footer_text, font=footer_font)
    w, h = bbox[2] - bbox[0], bbox[3] - bbox[1]
    draw.text(((card_width - w)//2, card_height - 30), footer_text, font=footer_font, fill="gray")
